...
## 2025-10-06: Исправление ошибки с передачей комиссии в стратегию

### Проблема
При запуске оптимизации возникала ошибка, связанная с передачей параметра комиссии в стратегию. Параметр `commission_pct` удалялся из `strategy_params`, но затем не передавался в стратегию при её создании.

### Причина
В методе `run_vectorized_klines_backtest` в `src/data/backtest_engine.py` параметры `initial_capital` и `commission_pct` удалялись из `strategy_params` для избежания конфликтов, но затем при создании стратегии использовался отфильтрованный словарь `filtered_params`, который не содержал эти параметры.

### Решение
1. Удалена фильтрация параметров `initial_capital` и `commission_pct` из `strategy_params`
2. При создании стратегии теперь передаются все параметры из `strategy_params` без фильтрации
3. Параметры `initial_capital` и `commission_pct` передаются явно при создании стратегии

### Изменения
- Исправлен метод `run_vectorized_klines_backtest` в `src/data/backtest_engine.py`
- Удалена переменная `filtered_params` и связанная с ней логика фильтрации
- Теперь все параметры стратегии передаются корректно, включая комиссию

### Статус
**ИСПРАВЛЕНО**: Ошибка с передачей комиссии в стратегию устранена, оптимизация должна работать корректно.
