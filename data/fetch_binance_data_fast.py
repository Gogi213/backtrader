...
async def fetch_binance_ohlcv_fast(symbol, interval, start, end, save_path=None):
    ...
    batch_sec = delta * 1000
    batch_ms = batch_sec * 1000
    tasks = []
    client = await AsyncClient.create(API_KEY, API_SECRET)
    curr = start_ts
    while curr < end_ts:
        batch_end = min(curr + batch_ms * 1000, end_ts)
        start_str = pd.to_datetime(curr, unit='ms').strftime('%d %b, %Y %H:%M:%S')
        end_str = pd.to_datetime(batch_end, unit='ms').strftime('%d %b, %Y %H:%M:%S')
        tasks.append(fetch_ohlcv_batch(client, symbol, interval, start_str, end_str))
        curr = batch_end + 1
...
if __name__ == '__main__':
    ...
    save_path = f'data/{symbol}_{interval}_{start}_{end}_fast.csv'
